...
const ClientLogo: React.FC<ClientLogoProps> = ({ src, alt }) => (
  <div className="flex-shrink-0 p-2 mx-4 flex items-center justify-center w-32 h-32">
    ...
    <Image
      src={src}
      alt={alt}
      width={alt === 'INDUS LOGO' ? 50 : 100} // Adjusted width for Indus logo
      height={alt === 'INDUS LOGO' ? 75 : 100} // Adjusted height for Indus logo
      className={cn(
        "object-contain transition-transform duration-300 ease-in-out hover:scale-110",
        alt === 'INDUS LOGO' ? 'w-[50px] h-[75px]' : 'w-[80px] h-[80px]' // Specific smaller box for Indus logo
      )}
